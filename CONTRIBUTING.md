...
For a control with **new IR data** also touch: `internal/ir/pipeline.go` plus the relevant collector under `collector/`. For **inventory-touching controls** (action/image attributes) also touch: `pbom/types.go`, `pbom/generate*.go`, `pbom/cyclonedx.go`, `cmd/analyze*.go::buildPBOMCompliance`.
...
- [ ] **IR field** — `internal/ir/pipeline.go`. Add the field to the right struct (`NormalizedPipeline`, `Job`, `Action`, `GitleaksHit`, …) with a JSON tag and a comment explaining what populates it. Every Rego rule reads from `input.pipeline.*`, so this is the contract.
  ...
  type Job struct {
      ...
      // ...
      // YourNewField is populated by collector/<your_collector>.go from
      // <data source>; empty when <degraded condition>.
      YourNewField []YourType `json:"yourNewField,omitempty"`
  }
  ```
- [ ] **Collector enrichment** — where the data comes from determines which file:
  - **GitLab API** → extend `collector/dataCollectionGitlab*.go` or write a new collector file. All REST/GraphQL calls live in `gitlab/`; collectors USE that, never call the API directly.
  - **GitHub workflow YAML parsing** → extend `collector/github_workflows.go`.
  - **GitHub repo metadata / Advisory DB / archived flag etc.** → extend `collector/github_metadata.go` (REST via `go-gh`) — reference: `advisoriesForRepo`, `resolveCommitToTag`.
  ...
  - **Repo-artifact scan** (Dockerfile bases, dependabot config) → extend `collector/github_repo_artifacts.go`.
- [ ] **Wiring** — call the collector from `control/task.go`'s `RunGitLabAnalysis` (GitLab) or `control/task_github.go`'s `RunGitHubAnalysis` (GitHub), after the pipeline is built and before the rego engine runs.
